...
class Filter:
    ...
    def filter_folder(self, path):
        ...
        image_paths = [os.path.join(path, f).replace("\\", "/")
                       for f in os.listdir(path) if f.lower().endswith(image_extensions)]
...
class MainApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Tkinter OOP példa — 3 részes elrendezés + kép elemző')
    ...
    def _create_main_frame(self):
        ...
        read_btn = ttk.Frame(frame); read_btn.pack(anchor='w', pady=4)
        self.read_path = tk.StringVar(value="")
        def get_path():
            self.read_path.set(filedialog.askdirectory(title="Olvasási mappa kiválasztása"))
    ...
    def run(self):
        Filter(settings).filter_folder(settings["read_path"])
...
if __name__ == '__main__':
    try:
        import PIL, numpy  # ellenőrzés
    except ImportError as e:
        msg = 'Hiányzó csomag: ' + str(e).split()[-1]
        tk.Tk().withdraw()
        messagebox.showerror('Függőség hiányzik', msg)
